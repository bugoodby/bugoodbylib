...
function ExcelDataCollector()
{
	...
	var m_tInfo = [
		{ "sheet": "URL_List", "row": 1, "col": 2 },
		{ "sheet": "URL_List", "row": 5, "col": 10 },
	];
	...
	this.init = function()
	{
		...
		for ( var i in m_tInfo ) {
			m_oOutSheet.Cells(HEADER_ROW, VALUE_START_COL + parseInt(i)).Value = m_tInfo[i].sheet + "/" + m_tInfo[i].row + "/" + m_tInfo[i].col;
	...
	var collectData = function( f )
	{
		...
		try {
			...
			for ( var i in m_tInfo ) {
				...
				var text = oSheet.Cells( m_tInfo[i].row, m_tInfo[i].col ).Text;
