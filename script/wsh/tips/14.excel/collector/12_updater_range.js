...
function ExecScript( filespec )
{
	...
	for ( var c = 1; c <= lastCol; c++ ) {
		...
		if ( text == "Path" ) {
			...
			var tmp = text.split("/");
			m_tInfo.push( { "sheet": tmp[0], "range": tmp[1], "valcol": c } );
		}
	}
	LOG("-----------------------");
	...
	for ( var r = HEADER_ROW + 1; r <= lastRow; r++ ) {
		...
		for ( var i in m_tInfo ) {
			...
			// 値の変更
			LOG(objTargetSheet.Range(m_tInfo[i].range).Text + " => " + val_after);
//			objTargetSheet.Range(m_tInfo[i].range).Value = val_after;
		}
